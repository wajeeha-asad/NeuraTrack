...
def _create_time_sensitive_reminders(db: Session, user: User):
    ...
    # Deadline reminders are evaluated when the notification center is opened.
    # LearningPath.deadline is stored as YYYY-MM-DD.
    ...
    for path in paths:
        ...
        if 0 <= days_left <= 3:
            ...
            create_notification(
                db, user, "Learning path deadline approaching ⏳", message,
                f"deadline:{path.id}:{deadline.isoformat()}", created_at=now,
            )

    # If the user has a streak and has not studied today, warn once per day.
